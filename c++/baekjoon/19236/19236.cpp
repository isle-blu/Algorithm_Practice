...
void move(int sum)
{
    ...
    for (int i = 1; i <= 16; i++)
    {
        ...
        if (pt.first + dx[now.direction] >= 4 || pt.first + dx[now.direction] < 0 || pt.second + dy[now.direction] >= 4 || pt.second + dy[now.direction] < 0)
        {
        ...
        fish_pt[i] = {pt.first + dx[now.direction], pt.second + dy[now.direction]};
    ...
    while (true)
    {
        if (move_pt.first >= 4 || move_pt.first < 0 || move_pt.second >= 4 || move_pt.second < 0)
        {
            if (ans < sum)
            {
                ans = sum;
        ...
        if (food != -1)
        {
            ...
            fishes[pt.first][pt.second].num = -1;
            fishes[pt.first][pt.second].direction = -1;
            fish_pt[0] = move_pt;
            fish_pt[food] = {-1, -1};
            move(sum + food);
...
int main()
{
    ...
    fish shark;
    shark.num = 0;
    shark.direction = fishes[0][0].direction;
    fish_pt[0] = {0, 0};
